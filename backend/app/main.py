...
async def lifespan(application: FastAPI):
    ...
    checkpointer_lifecycle = None
    queue_context = None
    job_queue = None
    if not settings.database_url.startswith("sqlite"):
        queue_context = procrastinate_app.open_async()
        await queue_context.__aenter__()
    ...
    try:
        ...
        if checkpointer_lifecycle is not None:
            await checkpointer_lifecycle.stop()
